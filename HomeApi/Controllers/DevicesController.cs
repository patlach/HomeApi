...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        public async Task<IActionResult> Delete(DeleteDeviceRequest request)
        {
            ...
            var device = await _devices.GetDeviceBySerialNumber(request.SerialNumber);
            if (device == null)
                return StatusCode(400, $"Ошибка: Устройство c номером {request.SerialNumber} не существует в {request.RoomLocation}.");

            await _devices.DeleteDevice(device);

            return StatusCode(201, $"Устройство {device.Name} удалено. Идентификатор: {request.SerialNumber}");
        }
