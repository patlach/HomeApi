...
namespace HomeApi.Controllers
{
    ...
    public class RoomsController : ControllerBase
    {
        ...
        [HttpPut]
        [Route("{roomName}")]
        public async Task<IActionResult> Edit([FromRoute] string roomName, [FromBody] EditRoomRequest request)
        {
            var room = await _repository.GetRoomByName(roomName);
            if (room == null)
                return StatusCode(400, $"Ошибка: Комната с именем {roomName} не существует.");
            ...
            return StatusCode(200, $"Комната была обновлена! Имя - {request.NewName}");
        }
    }
}